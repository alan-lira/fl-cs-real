...
def select_clients_using_fedaecs_adapted(comm_round: int,
    ...
    if not individual_metrics_history:
        ...
        for client_key, client_values in available_participating_clients_map.items():
                ...
                if assignment_capacity not in previous_num_tasks_assigned and \
                   assignment_capacity <= num_tasks_to_schedule:
                    ...
                    if prev_known_assignment != next_known_assignment:
                        ...
                        y1_accuracy = accuracy_gains_client[prev_known_assignment]
                        y2_accuracy = accuracy_gains_client[next_known_assignment]
                        accuracy_gain_estimation = calculate_linear_interpolation_or_extrapolation(prev_known_assignment,
                                                                                                   next_known_assignment,
                                                                                                   y1_accuracy,
                                                                                                   y2_accuracy,
                                                                                                   assignment_capacity)
            ...
            filtered_accuracy_gains_client = []
            for index in range(0, num_tasks_to_schedule + 1):
                if time_costs_client[index] != inf and energy_costs_client[index] != inf:
                    ...
                    filtered_accuracy_gains_client.append(accuracy_gains_client[index])
            ...
            accuracy_gains.append(filtered_accuracy_gains_client)
