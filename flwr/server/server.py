...
4) Line 369: Used the '_time_function_execution' function to measure the elapsed time of 'fit_client' function.
5) Lines 408-420: Addition that allows to get the 'communication_time' and 'client_id' metrics of 'fit' clients.
6) Line 441: Used the '_time_function_execution' function to measure the elapsed time of 'evaluate_client' function.
7) Lines 483-495: Addition that allows to get the 'communication_time' and 'client_id' metrics of 'evaluate' clients.
"""
...
def fit_clients(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        submitted_fs = {
            executor.submit(_time_function_execution(fit_client), client_proxy, ins, timeout, group_id)
            for client_proxy, ins in client_instructions
        }
        finished_fs, _ = concurrent.futures.wait(
            fs=submitted_fs,
...
def _handle_finished_future_after_fit(
    ...
    client_prompted = result[0].get_properties(gpi, timeout=None, group_id=None)
...
def evaluate_clients(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        submitted_fs = {
            executor.submit(_time_function_execution(evaluate_client), client_proxy, ins, timeout, group_id)
            for client_proxy, ins in client_instructions
        }
        finished_fs, _ = concurrent.futures.wait(
            fs=submitted_fs,
